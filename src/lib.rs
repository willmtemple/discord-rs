...
impl Discord {
    ...
    pub fn get_messages(&self,
        ...
        use std::fmt::Write;
        let mut url =
            format!(api_concat!("/channels/{}/messages?limit={}"), channel, limit.unwrap_or(50));
        match what {
    ...
    pub fn send_embed<F: FnOnce(EmbedBuilder) -> EmbedBuilder>(&self,
        ...
        let map = ObjectBuilder::new()
            .insert("content", text)
            .insert("embed",
                    EmbedBuilder::__build(f, Default::default()).build())
            .build();
        let body = try!(serde_json::to_string(&map));
        let response = request!(self, post(body), "/channels/{}/messages", channel);
        Message::decode(try!(serde_json::from_reader(response)))
    ...
    pub fn send_file<R: ::std::io::Read>(&self,
                                         ...
                                         -> Result<Message> {
        let url =
            match hyper::Url::parse(&format!(api_concat!("/channels/{}/messages"), channel)) {
                Ok(url) => url,
    ...
    /// Get the logged-in bot or user's profile.
    ...
    pub fn suggested_shard_count(&self) -> Result<u64> {
        ...
        let mut value: BTreeMap<String, serde_json::Value> =
        ...
        match value.remove("shards") {
            ...
            None => {
                return Err(Error::Decode("suggested_shard_count missing \"shards\"",
