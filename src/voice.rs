...
impl VoiceConnection {
    ...
    fn send_connect(&self) {
        let _ = self.main_ws.send(::internal::Status::SendMessage(ObjectBuilder::new()
            .insert("op", 4)
            .insert_object("d", |object| {
                object.insert("guild_id", self.server_id.map(|s| s.0))
...
impl InternalConnection {
    fn new(info: ConnStartInfo) -> Result<InternalConnection> {
        ...
        let map = ObjectBuilder::new()
            .insert("op", 0)
            .insert_object("d", |object| {
                object.insert("server_id", server_id)
...

        {
            ...
            let map = ObjectBuilder::new()
                .insert("op", 1)
                .insert_object("d", |object| {
                    object.insert("protocol", "udp")
                        .insert_object("data", |object| {
                            object.insert("address", own_address)
            ...
            match try!(receiver.recv_json(VoiceEvent::decode)) {
                VoiceEvent::Ready { mode, secret_key } => {
                    ...
                    if mode != "xsalsa20_poly1305" {
                        return Err(Error::Protocol("Voice mode in Ready was not \
                                                    \"xsalsa20_poly1305\""));
                    }
                    break;
                }
    ...
    fn update(&mut self,
        ...
        if let Some(receiver) = receiver.as_mut() {
            while let Ok(status) = self.receive_chan.try_recv() {
                match status {
                    RecvStatus::Websocket(VoiceEvent::SpeakingUpdate { user_id,
                                                                       ssrc,
                                                                       speaking }) => {
    ...
    fn set_speaking(&mut self, speaking: bool) -> Result<()> {
        ...
        let map = ObjectBuilder::new()
            .insert("op", 5)
            .insert_object("d", |object| {
                object.insert("speaking", speaking)
                    .insert("delay", 0)
            })
            .build();
