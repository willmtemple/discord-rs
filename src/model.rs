...
impl ServerInfo {
    pub fn decode(value: Value) -> Result<Self> {
        ...
        warn_json!(value,
                   ServerInfo {
                       ...
                       permissions: try!(remove(&mut value, "permissions")
    ...
    pub fn icon_url(&self) -> Option<String> {
        self.icon.as_ref().map(|icon| format!(cdn_concat!("/icons/{}/{}.jpg"), self.id, icon))
    }
}
    ...
    pub fn decode(value: Value) -> Result<Server> {
        ...
        warn_json!(value,
                   Server {
                       ...
                       embed_channel_id: try!(opt(&mut value,
                                                  "embed_channel_id",
                                                  ChannelId::decode)),
                       ...
                       verification_level: try!(remove(&mut value, "verification_level")
                           .and_then(VerificationLevel::decode)),
                       ...
                       default_message_notifications:
                           req!(try!(remove(&mut value, "default_message_notifications")).as_u64()),
...
impl ServerPrune {
    pub fn decode(value: Value) -> Result<ServerPrune> {
        let mut value = try!(into_map(value));
        warn_json!(value,
                   ServerPrune { pruned: req!(try!(remove(&mut value, "pruned")).as_u64()) })
    }
}
    ...
    pub fn decode(value: Value) -> Result<Role> {
        ...
        warn_json!(value,
                   Role {
                       ...
                       mentionable: try!(opt(&mut value, "mentionable", |v| Ok(req!(v.as_bool()))))
                           .unwrap_or(false),
                   })
    }
...
impl User {
    pub fn decode(value: Value) -> Result<User> {
        ...
        warn_json!(value,
                   User {
                       ...
                       discriminator: try!(remove(&mut value, "discriminator")
                           .and_then(decode_discriminator)),
                       avatar: try!(opt(&mut value, "avatar", into_string)),
                       bot: try!(opt(&mut value, "bot", |v| Ok(req!(v.as_bool()))))
                           .unwrap_or(false),
                   })
    }
    ...
    pub fn avatar_url(&self) -> Option<String> {
        self.avatar.as_ref().map(|av| format!(cdn_concat!("/avatars/{}/{}.jpg"), self.id, av))
    }
}
    ...
    pub fn decode(value: Value) -> Result<Group> {
        ...
        warn_json!(value,
                   Group {
                       ...
                       owner_id: try!(remove(&mut value, "owner_id").and_then(UserId::decode)),
                       recipients: try!(opt(&mut value,
                                            "recipients",
                                            |r| decode_array(r, User::decode)))
                           .unwrap_or(Vec::new()),
                   })
    }
    ...
    pub fn decode(value: Value) -> Result<Message> {
        ...
        warn_json!(value,
                   Message {
                       ...
                       mention_everyone: req!(try!(remove(&mut value, "mention_everyone"))
                           .as_bool()),
                       mentions: try!(decode_array(try!(remove(&mut value, "mentions")),
                                                   User::decode)),
                       mention_roles: try!(decode_array(try!(remove(&mut value, "mention_roles")),
                                                        RoleId::decode)),
                       author: try!(remove(&mut value, "author").and_then(User::decode)),
                       attachments: try!(decode_array(try!(remove(&mut value, "attachments")),
                                                      Attachment::decode)),
                       embeds: try!(decode_array(try!(remove(&mut value, "embeds")), Ok)),
                       reactions: try!(opt(&mut value,
                                           "reactions",
                                           |x| decode_array(x, MessageReaction::decode)))
                           .unwrap_or(Vec::new()),
                   })
    }
}
    ...
    pub fn decode(value: Value) -> Result<VoiceRegion> {
        ...
        warn_json!(value,
                   VoiceRegion {
                       id: try!(remove(&mut value, "id").and_then(into_string)),
                       name: try!(remove(&mut value, "name").and_then(into_string)),
                       sample_hostname: try!(remove(&mut value, "sample_hostname")
    ...
    pub fn decode(value: Value) -> Result<ReadState> {
        ...
        warn_json!(value,
                   ReadState {
                       id: try!(remove(&mut value, "id").and_then(ChannelId::decode)),
                       last_message_id: try!(opt(&mut value, "last_message_id", MessageId::decode)),
                       mention_count: try!(opt(&mut value,
                                               "mention_count",
                                               |v| Ok(req!(v.as_u64()))))
                           .unwrap_or(0),
                   })
        ...
        let kind = match value.remove("type") {
            ...
            Some(Value::String(v)) => {
                match v.parse::<u64>() {
                    Ok(v) => Value::U64(v),
                    Err(_) => {
                        return Err(Error::Decode("Expected valid GameType", Value::String(v)))
                    }
                }
            }
    ...
    pub fn permissions_for(&self, channel: ChannelId, user: UserId) -> Permissions {
        ...
        if text_channel {
            permissions &= !(VOICE_CONNECT | VOICE_SPEAK | VOICE_MUTE_MEMBERS |
                             ...
                             VOICE_MOVE_MEMBERS | VOICE_USE_VAD);
        }
        permissions
    }
}
...
impl RelationshipType {
    ...
    fn decode(value: Value) -> Result<Self> {
        value.as_u64()
            .and_then(RelationshipType::from_num)
            .ok_or(Error::Decode("Expected valid RelationshipType", value))
    }
}
...
impl GatewayEvent {
    pub fn decode(value: Value) -> Result<Self> {
        ...
        match req!(value.get("op").and_then(|x| x.as_u64())) {
            0 => {
                Ok(GatewayEvent::Dispatch(req!(try!(remove(&mut value, "s")).as_u64()),
                                          try!(Event::decode(
...
fn opt<T, F: FnOnce(Value) -> Result<T>>(map: &mut BTreeMap<String, Value>,
                                         key: &str,
                                         f: F)
                                         -> Result<Option<T>> {
...
fn decode_discriminator(value: Value) -> Result<u16> {
    match value {
        ...
        Value::String(s) => {
            s.parse::<u16>().or(Err(Error::Other("Error parsing discriminator as u16")))
        }
