...
impl Connection {
            ...
            match self.receiver.recv_json(GatewayEvent::decode) {
                ...
                Ok(GatewayEvent::Dispatch(sequence, event)) => {
					...
					#[cfg(feature="voice")]                    {
                        ...
                        if let Event::VoiceServerUpdate { server_id,
                                                          channel_id: _,
                                                          ref endpoint,
                                                          ref token } = event {
    ...
    fn resume(&mut self, session_id: String) -> Result<Event> {
        ...
        let resume = ObjectBuilder::new()
            .insert("op", 6)
            .insert_object("d", |o| {
                o.insert("seq", self.last_sequence)
    ...
    pub fn __download_members(&self, servers: &[ServerId]) {
        let msg = ObjectBuilder::new()
            .insert("op", 8)
            .insert_object("d", |o| {
                o.insert_array("guild_id", |a| servers.iter().fold(a, |a, s| a.push(s.0)))
                    .insert("query", "")
                    .insert("limit", 0)
            })
            .build();
    ...
    pub fn sync_calls(&self, channels: &[ChannelId]) {
        for &channel in channels {
            let msg = ObjectBuilder::new()
                .insert("op", 13)
                .insert_object("d", |o| o.insert("channel_id", channel.0))
                .build();
            let _ = self.keepalive_channel.send(Status::SendMessage(msg));
        }
    }
...
fn identify(token: &str, shard_info: Option<[u8; 2]>) -> serde_json::Value {
    ObjectBuilder::new()
        ...
        .insert_object("d", |mut object| {
            object = object.insert("token", token)
                .insert_object("properties", |object| {
                    object.insert("$os", ::std::env::consts::OS)
            ...
            if let Some(shard_info) = shard_info {
                object = object.insert_array("shard",
                                             |array| array.push(shard_info[0]).push(shard_info[1]));
            }
