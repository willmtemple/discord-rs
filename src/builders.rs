...
impl EditMember {
    ...
    pub fn roles(self, roles: &[RoleId]) -> Self {
        EditMember(self.0.insert_array("roles", |ab| roles.iter().fold(ab, |ab, id| ab.push(id.0))))
    }
...
impl EmbedBuilder {
    ...
    pub fn footer<F: FnOnce(EmbedFooterBuilder) -> EmbedFooterBuilder>(self, f: F) -> Self {
        EmbedBuilder(self.0.insert("footer",
                                   f(EmbedFooterBuilder(ObjectBuilder::new())).0.build()))
    }
    ...
    pub fn author<F: FnOnce(EmbedAuthorBuilder) -> EmbedAuthorBuilder>(self, f: F) -> Self {
        EmbedBuilder(self.0.insert("author",
                                   f(EmbedAuthorBuilder(ObjectBuilder::new())).0.build()))
    }
    ...
    pub fn fields<F: FnOnce(EmbedFieldsBuilder) -> EmbedFieldsBuilder>(self, f: F) -> Self {
        EmbedBuilder(self.0.insert("fields",
                                   f(EmbedFieldsBuilder(ArrayBuilder::new())).0.build()))
    }
}
...
impl EmbedFieldsBuilder {
    ...
    pub fn field(self, name: &str, value: &str, inline: bool) -> Self {
        EmbedFieldsBuilder(self.0.push(ObjectBuilder::new()
            .insert("name", name)
            .insert("value", value)
            .insert("inline", inline)
            .build()))
    }
}
